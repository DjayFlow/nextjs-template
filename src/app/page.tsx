...
const CREDIT_LIMIT_LEVEL = 200; // De grens waar credits stoppen
...
export default function Home() {
  ...
  // --- TON PAYMENT LOGIC ---
  const payWithTon = async (amount: string) => {
    if (!tonConnectUI.connected) {
        setEventMsg("❌ CONNECT WALLET FIRST!");
        return;
    }
    ...
    const transaction = {
        ...
        messages: [{
            address: "JOUW_TON_WALLET_ADRES", // VERVANG DIT DOOR JE EIGEN ADRES!
            amount: amount, // NanoTON (1 TON = 1.000.000.000)
        }]
    };
    ...
    try {
        ...
        setStage(s => s + 1);
        setEventMsg("💎 DIVINE UPGRADE COMPLETE!");
        new Audio('/sounds/win.mp3').play().catch(() => {});
    } catch (e) {
        setEventMsg("❌ TRANSACTION FAILED");
    }
  };

  // --- UPGRADE HANDLER (CREDITS vs TON) ---
  const handleUpgrade = () => {
    if (stage < CREDIT_LIMIT_LEVEL) {
        ...
        // Hier start de TON betaling voor level 200+
        payWithTon("500000000"); // 0.5 TON per level
    }
  };
  ...
  const renderHome = () => (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', position: 'relative' }}>
      ...
      <div style={{ margin: '5px 0', height: '160px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
         ...
         <div style={{ backgroundColor: stage >= CREDIT_LIMIT_LEVEL ? '#00ffcc' : '#ffcc00', color: 'black', padding: '2px 12px', borderRadius: '10px', fontSize: '10px', fontWeight: '900', marginTop: '5px' }}>
            LVL {stage} | {stage >= CREDIT_LIMIT_LEVEL ? 'DIVINE' : 'UNBREAKABLE'}
         </div>
      </div>
                 ...
                 view === 'shop' ? (
                    <div style={{ width: '100%', padding: '10px' }}>
                        ...
                        <Section header="UPGRADES">
                            {stage < CREDIT_LIMIT_LEVEL ? (
                                <Cell subtitle={`${(stage * 10000).toLocaleString()} Credits`} after={<Button size="s" onClick={handleUpgrade}>LVL UP</Button>}>EVOLVE OWL</Cell>
                            ) : (
                        ...
                        <Section header="ECONOMY" footer="Credits can only be used until Level 200.">
                    ...
                    <div style={{ width: '100%', textAlign: 'center', padding: '20px' }}>
                        <Headline style={{ color: '#ffcc00' }}>{view.toUpperCase()} HUB</Headline>
