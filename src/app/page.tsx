  ...
  const handleUpgrade = async () => {
    if (stage < CREDIT_LIMIT_LEVEL) {
        ...
        const transaction = {
            ...
            messages: [{ address: "JOUW_TON_ADRES", amount: "500000000" }] // 0.5 TON
        };
        try {
  ...
  const renderHome = () => (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', position: 'relative' }}>
      ...
      <div style={{ margin: '5px 0', height: '160px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
         ...
         <div style={{ backgroundColor: stage >= CREDIT_LIMIT_LEVEL ? '#00ffcc' : '#ffcc00', color: 'black', padding: '2px 12px', borderRadius: '10px', fontSize: '10px', fontWeight: '900', marginTop: '5px' }}>LVL {stage} | {stage >= CREDIT_LIMIT_LEVEL ? 'DIVINE' : 'UNBREAKABLE'}</div>
      </div>
                 ...
                 view === 'radar' ? (
                    ...
                    <div style={{ width: '100%', padding: '10px' }}>
                        ...
                        <Section header="UPGRADES">
                            <Cell subtitle={stage < CREDIT_LIMIT_LEVEL ? `${(stage * 10000).toLocaleString()} Credits` : "0.5 TON"} after={<Button size="s" onClick={handleUpgrade}>UPGRADE</Button>}>LEVEL UP</Cell>
