...
const CREDIT_LIMIT = 200; // Jouw regel: tot level 200 met credits

export default function Home() {
  const [tonConnectUI] = useTonConnectUI();
  
  // --- STATE (Uit jouw eigen logs) ---
  ...
  // FIX: Meldingen gaan na 3 seconden weg
  useEffect(() => {
    if (eventMsg !== '') {
  ...
  const handleUpgradeClick = async () => {
    if (stage < CREDIT_LIMIT) {
      ...
      if (points >= cost) {
        ...
        setEventMsg("❌ NO CREDITS");
      }
    } else {
      // TON BETALING (Level 200+)
      ...
      const transaction = {
        ...
        messages: [{
          address: "UQCHoXWzQZgYjE_9j_l_7pB08M9Y7M7N7O7P7Q7R7S7T", // JE EIGEN WALLET ADRES
                 ...
                 view === 'radar' ? (
                    <div style={{ width: '100%', padding: '10px' }}>
                        ...
                        <Section header="TASKS">
                            ...
                            <Cell subtitle="+5.000 Credits" after={<Button size="s" disabled={claimedQuests.includes('unity')} onClick={() => handleQuest('unity', 5000, 'https://t.me/your_channel')}>JOIN</Button>}>Join Community</Cell>
                        </Section>
                    ...
                    <div style={{ width: '100%', padding: '10px' }}>
                        ...
                        <Section header="UPGRADES">
                            {stage < CREDIT_LIMIT ? (
                                <Cell subtitle={`${(stage * 10000).toLocaleString()} Credits`} after={<Button size="s" onClick={handleUpgradeClick}>UPGRADE</Button>}>EVOLVE OWL</Cell>
                            ) : (
                                <Cell subtitle="0.5 TON Required" after={<Button size="s" style={{backgroundColor:'#00ffcc', color:'black'}} onClick={handleUpgradeClick}>DIVINE UPGRADE</Button>}>LEVEL {stage} ➔ {stage+1}</Cell>
                            )}
                    ...
                    <div style={{ width: '100%', textAlign: 'center' }}>
                        ...
                        <Section header="RECRUIT"><Title style={{ color: '#ffcc00' }}>0</Title><Button style={{ width: '100%', marginTop: '10px' }} onClick={() => { navigator.clipboard.writeText("https://t.me/your_bot"); setEventMsg("COPIED!"); }}>INVITE FRIEND</Button></Section>
