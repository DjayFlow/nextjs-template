...
const CREDIT_LIMIT = 200; // Jouw regel: tot 200 met credits, daarna TON
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Auto-clear meldingen na 3 seconden
    if (eventMsg !== '') {
      const timer = setTimeout(() => setEventMsg(''), 3000);
      return () => clearTimeout(timer);
    }
  }, [points, spins, stage, lastGift, bossHp, claimedQuests, isLoaded, eventMsg]);
  ...
  const handleTonUpgrade = async () => {
    ...
    const transaction = {
      ...
      messages: [{
        address: "UQCHoXWzQZgYjE_9j_l_7pB08M9Y7M7N7O7P7Q7R7S7T", // JE EIGEN WALLET
    ...
    try {
      ...
      setStage(s => s + 1);
      setEventMsg("💎 DIVINE UPGRADE SUCCESS!");
  ...
  const renderHome = () => (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', position: 'relative' }}>
      <div style={{ position: 'absolute', left: '10px', top: '90px', display: 'flex', flexDirection: 'column', gap: '15px', zIndex: 100 }}>
      ...
      <div style={{ margin: '5px 0', height: '160px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
         ...
         <div style={{ backgroundColor: stage >= CREDIT_LIMIT ? '#00ffcc' : '#ffcc00', color: 'black', padding: '2px 10px', borderRadius: '10px', fontSize: '10px', fontWeight: '900', marginTop: '5px' }}>LVL {stage} | {stage >= CREDIT_LIMIT ? 'DIVINE' : 'UNBREAKABLE'}</div>
      </div>
            ...
            <div style={{ width: '100%', flex: 1 }}>
                ...
                {view === 'shop' && (
                    <div style={{ padding: '10px' }}>
                        ...
                        <Section header="UPGRADES" footer={stage >= CREDIT_LIMIT ? "Divine levels require TON." : "Use credits until level 200."}>
                            {stage < CREDIT_LIMIT ? (
                                <Cell subtitle={`${(stage * 10000).toLocaleString()} Credits`} after={<Button size="s" onClick={handleUpgradeClick}>UPGRADE</Button>}>LEVEL UP</Cell>
                            ) : (
                                <Cell subtitle="0.5 TON" after={<Button size="s" mode="filled" style={{backgroundColor:'#00ffcc', color:'black'}} onClick={handleTonUpgrade}>DIVINE UPGRADE</Button>}>DIVINE LEVEL UP</Cell>
                            )}
                ...
                {view === 'friends' && (
                    <div style={{ textAlign: 'center', padding: '10px' }}>
                        ...
                        <Section header="UNITY FLEET"><Cell subtitle="Invite for 100 Energy!">0 Recruits</Cell><Button style={{ width: '100%', marginTop: '10px' }} onClick={() => { navigator.clipboard.writeText("https://t.me/your_bot"); setEventMsg("LINK COPIED!"); }}>INVITE</Button></Section>
                ...
                {view === 'radar' && (
                    <div style={{ padding: '10px' }}>
                        ...
                        <Section header="MISSIONS"><Cell after={<Button size="s" disabled={claimedQuests.includes('daily')} onClick={() => { setPoints(p=>p+1000); setClaimedQuests([...claimedQuests, 'daily']); setEventMsg("CLAIMED!"); }}>{claimedQuests.includes('daily') ? 'DONE' : 'CLAIM'}</Button>}>Daily Login</Cell></Section>
                        <Button onClick={() => setView('home')} style={{ marginTop: '20px', width: '100%' }}>BACK</Button>
                    </div>
                )}
                {view === 'boss' && (
